...
export function ReservaForm({ nombreUsuario }: ReservaFormProps) {
  ...
  const { origen, destino, paradas, puntoActivo, reiniciar, setPunto } =
    useReservaBorrador()
  ...
  useEffect(() => {
    if (origen)
      form.setValue("origen", origen, {
        shouldValidate: form.formState.isSubmitted,
      })
  }, [origen, form])
  useEffect(() => {
    if (destino)
      form.setValue("destino", destino, {
        shouldValidate: form.formState.isSubmitted,
      })
  }, [destino, form])
  useEffect(() => {
    form.setValue(
      "paradas",
      paradas.filter((point) => point !== null)
    )
  }, [paradas, form])
  ...
  const timingFields = (
    <Field className="rounded-2xl bg-card p-4 shadow-sm">
      ...
      <Tabs
        value={tipo}
        onValueChange={(value) =>
          form.setValue("tipo", value as CrearReservaInput["tipo"])
        }
      >
  ...
  const notesField = (
    <FormTextareaControl
      ...
      icon={RiFileTextLine}
      error={errores.notas}
      fieldClassName="rounded-2xl bg-card p-4 shadow-sm"
      id="notas"
      rows={3}
  ...
  return (
    <form
      ...
      onSubmit={onSubmit}
      noValidate
      className="h-full max-w-full min-w-0 overflow-hidden lg:h-auto lg:pb-4"
    >
      <FieldGroup className="h-full gap-5 lg:h-auto">
        ...
        <Field className="h-full min-h-0 max-w-full min-w-0 overflow-hidden lg:h-auto">
          <div className="hidden pb-2 lg:block">
            <FieldLabel className="text-base">Diseña tu ruta</FieldLabel>
            <p className="mt-1 text-sm text-muted-foreground">
              Busca el origen y el destino. Puedes ajustar cada punto desde las
              opciones del viaje.
            </p>
          </div>
          <div className="relative h-full lg:h-auto">
            ...
            {!isDesktop && (
              <Sheet>
                <SheetTrigger
                  render={
                    <Button
                      type="button"
                      variant="secondary"
                      className="absolute bottom-20 left-1/2 z-30 h-auto -translate-x-1/2 justify-start gap-3 rounded-2xl bg-primary px-2 py-2 text-primary-foreground shadow-[0_12px_32px_rgb(30_29_26/22%)] hover:bg-primary/90"
                    />
                  }
                >
                ...
                <SheetContent
                  side="bottom"
                  className="max-h-[calc(100dvh-1rem)] overflow-hidden rounded-t-3xl border-0 p-0"
                >
                  <SheetHeader className="shrink-0 px-5 pt-5 pb-3 text-left">
                    <SheetTitle>Configura tu viaje</SheetTitle>
                    <SheetDescription>
                      Elige el recorrido y revisa los detalles antes de
                      solicitar.
                    </SheetDescription>
                  </SheetHeader>
                  <div className="min-h-0 flex-1 space-y-4 overflow-y-auto overscroll-contain px-4 pb-[max(1.5rem,env(safe-area-inset-bottom))]">
                    <div className="sticky top-0 z-10 bg-popover py-1">
                      <DireccionSearch
                        ...
                        onSelect={(resultado) =>
                          setPunto(puntoActivo, resultado)
                        }
                      />
                    </div>
                    {timingFields}
          ...
          <FieldError
            errors={[
              errores.origen
                ? { message: "Fija el origen en el mapa" }
                : undefined,
              errores.destino
                ? { message: "Fija el destino en el mapa" }
                : undefined,
            ]}
          />
        </Field>

...
function DashboardNavigationControls({
  ...
  const mobileItems = items
    .filter(
      (item) =>
        !item.href.endsWith("/notifications") && !item.href.endsWith("/profile")
    )
    .slice(0, 4)
  ...
  return (
    <>
      <button
        ...
        className="fixed top-8 left-[calc(var(--sidebar-width)-0.55rem)] z-30 hidden size-10 place-items-center rounded-md bg-card text-foreground shadow-lg shadow-black/10 transition-[left,top,opacity,transform,background-color] duration-300 peer-data-[state=collapsed]:top-7 peer-data-[state=collapsed]:left-2.5 hover:scale-105 hover:bg-primary focus-visible:bg-primary focus-visible:opacity-100 focus-visible:outline-none md:grid"
      >
      ...
      <nav
        ...
        className={cn(
          "fixed bottom-1.5 left-1/2 z-40 grid w-full max-w-[calc(100%-0.375rem)] -translate-x-1/2 overflow-hidden rounded-3xl bg-secondary/95 p-2 text-secondary-foreground shadow-2xl shadow-black/25 backdrop-blur-xl md:hidden",
...
function MobileSidebarButton() {
  ...
  return (
    <button
      type="button"
      onClick={toggleSidebar}
      className="flex min-h-8 items-center gap-2 rounded-xl bg-secondary px-2 text-xs font-medium text-secondary-foreground shadow-sm active:scale-95 md:hidden"
...
export function DashboardShell({
  ...
  const profileHref = portal.nav.find((item) =>
    item.href.endsWith("/profile")
  )?.href
  const notificationsHref = portal.nav.find((item) =>
    item.href.endsWith("/notifications")
  )?.href
  const unreadNotifications = trpc.notificaciones.contarNoLeidas.useQuery(
    undefined,
    {
  ...
  const isBookingPage = pathname === "/customer/book"
  const active = (href: string) =>
    normalizedPath === href ||
    (href !== portal.home && normalizedPath.startsWith(`${href}/`))
  ...
  return (
    <SidebarProvider
      className={cn(
        "min-h-dvh min-w-0 overflow-x-hidden bg-secondary p-0 sm:p-2 md:p-3",
        isBookingPage && "max-md:h-dvh max-md:min-h-0 max-md:overflow-hidden"
      )}
    >
      <Sidebar
        collapsible="icon"
        className="border-0 border-none! bg-secondary text-secondary-foreground group-data-[collapsible='icon']:pl-1 group-data-[state=expanded]:w-[calc(var(--sidebar-width)+1px)]"
      >
        <SidebarHeader className="px-3 py-5">
          <SidebarMenu>
            <SidebarMenuItem>
              <SidebarMenuButton
                size="lg"
                render={<Link href={hrefFor(portal.home)} />}
              >
          ...
          <Collapsible className="group/account mt-2 md:hidden">
            <CollapsibleTrigger className="flex min-h-16 w-full items-center gap-3 rounded-2xl bg-white/6 p-3 text-left transition-colors outline-none focus-visible:bg-white/10 active:bg-white/10">
              ...
              <RiArrowDownSLine
                className="size-4 shrink-0 text-sidebar-foreground/55 transition-transform group-data-open/account:rotate-180"
                aria-hidden
              />
            </CollapsibleTrigger>
            <CollapsibleContent className="h-(--collapsible-panel-height) overflow-hidden transition-[height] duration-200 data-closed:h-0">
              <div className="space-y-3 px-2 pt-3">
                <div className="rounded-xl bg-white/4 px-3 py-2.5">
                  ...
                  <p className="mt-1 truncate text-sm text-sidebar-foreground/80">
                    {user.email}
                  </p>
                </div>
                <SidebarMenu className="gap-1">
                  {profileHref && (
                    <SidebarMenuItem>
                      <SidebarMenuButton
                        render={<Link href={hrefFor(profileHref)} />}
                      >
                    ...
                    availablePortals.map((destination) => (
                      <SidebarMenuItem key={destination.id}>
                        ...
                        >
                          ...
                          <span>
                            {destination.id === portalId
                              ? `${destination.title} · Panel actual`
                              : `Cambiar a ${destination.title}`}
                          </span>
                        </SidebarMenuButton>
                      </SidebarMenuItem>
                    ))}
                ...
                {portal.nav.map((item) => (
                  ...
                  >
                    <SidebarMenuButton
                      ...
                      className="group/menubtn h-12 text-sidebar-foreground/75 transition-all hover:ps-6 hover:text-primary data-active:bg-primary/20 data-active:ps-4! data-active:text-primary"
                    >
                      <div className="absolute left-0 hidden h-4 w-1 rounded-full bg-primary group-data-[collapsible='icon']:hidden! group-data-active/menubtn:block" />
      ...
      <SidebarInset
        className={cn(
          "min-h-[calc(100dvh-1rem)] overflow-hidden bg-background shadow-2xl shadow-black/15 sm:rounded-3xl md:min-h-[calc(100dvh-1.5rem)]",
          isBookingPage && "max-md:h-dvh max-md:min-h-0"
        )}
      >
        <header className="sticky top-0 z-20 flex h-15 items-center gap-3 bg-background/90 px-1.5 shadow-[0_8px_30px_rgba(0,0,0,0.025)] backdrop-blur-xl sm:h-18 sm:px-4 md:px-6 md:ps-10!">
          <MobileSidebarButton />
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-64 rounded-2xl p-2">
                <DropdownMenuGroup>
                  ...
                  {profileHref && (
                    <DropdownMenuItem
                      onClick={() => router.push(hrefFor(profileHref))}
                    >
        ...
        <main
          ...
          className={cn(
            "min-h-0 flex-1",
            isBookingPage
              ? "overflow-hidden p-0 md:overflow-y-auto md:p-6 lg:p-8"
              : "overflow-y-auto p-1.5 pb-28 md:p-6 lg:p-8"
          )}
        >
          {children}
        </main>
